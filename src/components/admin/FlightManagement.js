...
const FlightManagement = () => {
  ...
  const flightLocationData = useSelector(
  ...
  const [fleetInformationModalVisible, setFleetInformationModalVisible] =
    useState(false);
  ...
  const edit = (record) => {
    form.setFieldsValue({
      ...record,
      departureAndArrival: [
        dayjs(record.departureTime),
        dayjs(record.arrivalTime),
      ],
    });
  ...
  const handleFlightFleetUpdate = (updatedFleet) => {
    ...
    const flightIndex = existingFlightData.findIndex(
      (flight) => flight.id === flightFleetEditingId
    );
  ...
  const columns = [
    {
      ...
      editable: true,
      render: (text, record) =>
        flightLocationData.find((loc) => loc.id === record.departureLocation)
          ?.airportName || "N/A",
    },
    {
      ...
      editable: true,
      render: (text, record) =>
        flightLocationData.find((loc) => loc.id === record.arrivalLocation)
          ?.airportName || "N/A",
    },
    {
      ...
      render: (_, record) => {
        return (
          <div>
            {formatDate(record.departureTime)} <br /> <FlightTakeoffIcon />{" "}
            <br /> {formatDate(record.arrivalTime)}
          </div>
        );
      },
    },
    {
      ...
      render: (_, record) => (
        <a
          href={() => false}
          disabled={flightFleetEditingId !== ""}
          onClick={() => showFleetModal(record)}
        >
          View
        </a>
      ),
      ...
      render: (_, record) => {
        ...
        return editable ? (
          ...
          <span>
            ...
            >
              <a href={() => false} disabled={flightEditingId !== ""}>
                Delete
              </a>
            </Popconfirm>
          </span>
  ...
  const mergedColumns = columns.map((col) => {
    ...
    return {
      ...
      onCell: (record) => ({
        ...
        title: col.title,
        inputType:
          col.dataIndex === "departureLocation" ||
          col.dataIndex === "arrivalLocation"
            ? "select"
            : col.dataIndex === "departureAndArrival"
            ? "rangePicker"
            : "text",
        record,
        selectOptions: flightLocationData.map(
          ({ id, airportName, cityName, country }) => ({
            label: `${airportName} at ${cityName}, ${country}`,
            value: id,
          })
        ),
        dependencies: ["departureLocation", "arrivalLocation"].filter(
          (field) => field !== col.dataIndex
        ),
        form,
      }),
    };
